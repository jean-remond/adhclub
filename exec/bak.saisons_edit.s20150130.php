...
function exec_saisons_edit_dist(){
	...
	if ((!$new AND !$row)
	  ...
	  OR (!$new AND (!autoriser('modifier','adhsaison', $id_saison))) 
		...
		echo minipres(_T('adhsaison:info_aucun_adhsaison'));
	} 
	else {
...
function saisons_edit($id_saison, $new, $config_fonc, $row){
	...
	echo $commencer_page(intval($id_saison)?_T('adhsaison:titre_adhsaison_cadre_modifier'):_T('adhsaison:icone_creer_adhsaison'), "naviguer", "saisons", 0);
	...
	$contexte = array(
	'icone_retour'=>icone_inline(_T('icone_retour'), $oups, _DIR_PLUGIN_ADHCLUB."/prive/themes/spip/images/adhsaison-24.png", "rien.gif",$GLOBALS['spip_lang_left']),
